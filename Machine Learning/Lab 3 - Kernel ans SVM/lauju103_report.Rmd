...
stations <- read.csv("stations.csv", sep=",", header=TRUE, stringsAsFactors=FALSE, fileEncoding="latin1")
...
st$dif_time <- abs(as.numeric(difftime(strptime(st$time_of_interest,"%H:%M:%S"),strptime(st$time,"%H:%M:%S") ,units = "hours")))
...
$$k_i(x,x')= exp\Bigg(-\bigg(\frac{||x-x'||}{h_i}\bigg)^2\Bigg)$$
...
df <- data.frame("Time"= result$time_of_interest ,"Predictions"= c(result$pred_sum_low, result$pred_sum_high), "Width"= rep(c("Low", "High"), each=11))
...
df <- data.frame("Time"= result$time_of_interest ,"Predictions"= c(result$pred_sum, result$pred_prod), "Model"= rep(c("Additive", "Multiplicative"), each=11))
...
The parameter C is the cost of constraints violation, it tells the SVM optimization how much we want to avoid misclassifying. It is useful to controll the outliers: if we define a low C we will allow more outliers, and for a high C fewer outliers will be allowed. 
...
+ geom_point(aes(color=Model)) + ggtitle("Predictions using different combinations of kerenls") 
+ theme_minimal() + theme(axis.text.x =element_text(size = rel(0.9), angle = 45))
```
