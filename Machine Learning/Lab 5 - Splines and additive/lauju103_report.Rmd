...
[^1]:When $\alpha < 0.05$ we reject $H_0:\beta_i=0$ which expresses that the coefficient $i$ is not significant (equals 0).
...
cat("The number of contributing features in the elastic net model is:", length(contributing_net), "\n")

contributing_svm <- colnames(data)[SVindex(svm_fit)]
coeff_svm <- coef(svm_fit)[[1]]
cat("The number of contributing features in the SVM model is:", length(contributing_svm), "\n")
...
df <- as.data.frame(cbind(c("NSC", "Elastic net", "SVM"), c(170,32,43), c(0.3,0.3,0.25)))
colnames(df) <- c("Model", "Number of features", "Test error")
df
```

The Support vector machine with “vanilladot” kernel method is the best model: it has a few more features than the Elastic net model but the misclassification error for the test data is 0.05 units lower. The nearest shrunken centroid classification method is the worst among these three models.
...
rejected <- sort_pval[which(sort_pval$pvalues < sort_pval$critica),1:2]
library(rowr)
